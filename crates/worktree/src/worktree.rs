...
impl Worktree {
    ...
    pub fn full_path(&self, worktree_relative_path: &RelPath) -> Arc<RelPath> {
...
impl Snapshot {
    ...
    pub fn absolutize(&self, path: &RelPath) -> PathBuf {
        if path.file_name().is_some() {
            self.abs_path.as_path().join(path.as_std_path())
        } else {
        ...
        for entry in removed_entries.cursor::<()>(&()) {
            ...
            if entry.path.file_name() == Some(GITIGNORE) {
                let abs_parent_path = self
                    ...
                    .abs_path
                    .as_path()
                    .join(entry.path.parent().unwrap().as_std_path());
...
impl language::File for File {
    ...
    fn full_path(&self, cx: &App) -> Arc<RelPath> {
...
pub struct Entry {
    ...
    pub canonical_path: Option<Arc<AbsPath>>,
...
impl Entry {
    fn new(
        ...
        root_char_bag: CharBag,
        canonical_path: Option<Arc<AbsPath>>,
    ) -> Self {
        ...
        abs_paths.retain(|abs_path| {
            ...
            let snapshot = &self.state.lock().snapshot;
            {
                ...
                if let Some((dot_git_abs_path, path_in_git_dir)) = dot_git_paths {
                    if skipped_files_in_dot_git.iter().any(|skipped| skipped.as_ref() == path_in_git_dir.as_path()) || skipped_dirs_in_dot_git.iter().any(|skipped_git_subdir| path_in_git_dir.starts_with(skipped_git_subdir)) {
        ...
        let (scan_job_tx, scan_job_rx) = channel::unbounded();
        {
            ...
            for path in paths {
                for ancestor in path.ancestors() {
                    ...
                    {
                        let abs_path = root_path.as_path().join(ancestor.as_std_path());
                        state.enqueue_scan_dir(
                            abs_path.into(),
                            entry,
                            &scan_job_tx,
...
impl<'a> TryFrom<(&'a CharBag, &PathMatcher, proto::Entry, PathStyle)> for Entry {
    ...
    fn try_from(
        ...
        Ok(Entry {
            ...
            canonical_path: if let Some(canonical_path) = entry.canonical_path {
                Some(Arc::new(AbsPath::new(path_style, &canonical_path)?))
            } else {
                None
            },
            is_ignored: entry.is_ignored,
